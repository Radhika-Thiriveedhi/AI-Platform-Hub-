"""Usage metering and quota accounting for AI Platform Hub.
...
class UsageRecord:
    """A durable-looking in-memory record used by the usage service."""
    identifier: str
    name: str
    status: str = "active"
...
class UsageService:
    """Coordinate usage operations with deterministic business rules."""
    ...
    def create(self, identifier: str, name: str, metadata: Mapping[str, Any] | None = None) -> dict[str, Any]:
        ...
        record = UsageRecord(identifier=identifier, name=name, metadata=dict(metadata or {}))
